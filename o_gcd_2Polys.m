function [] = o_gcd_2Polys(ex_num,emin,emax,mean_method, bool_alpha_theta, low_rank_approx_method, apf_method)
...
% % Example
%
% >> o_gcd_2Polys('1',1e-12,1e-10,'Geometric Mean Matlab Method', 'y','Standard STLN','Standard APF Nonlinear')
%
% >> o_gcd_2Polys(ex_num,1e-12,1e-10,'Geometric Mean Matlab Method', 'y','Standard STLN', 'Standard APF Nonlinear')
...
error.dx = GetDistance(dx_exact,dx_calc);
error.ux = GetDistance(ux_exact,ux_calc);
error.vx = GetDistance(vx_exact,vx_calc);
error.MyMethod = error.dx;
...
PrintToFile(GetDegree(fx),GetDegree(gx),GetDegree(dx_exact),GetDegree(dx_calc),error)
